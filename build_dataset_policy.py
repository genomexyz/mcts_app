...
while True:
    ...
    for iter_hist in range(len(all_hist)):
        ...
        for iter_next in range(len(next_move)):
            ...
            label = np.array([1])
            if len(all_input) == 0:
                all_input = np.reshape(input_feature, (1, -1))
                all_label = label
            else:
                all_input = np.concatenate([all_input, np.reshape(input_feature, (1, -1))])
...
np.save('feature_policy_new.npy', all_input)
np.save('label_policy_new.npy', all_label)